...
int socket(int domain, int type, int protocol)
{
   ...
   ret = wiznet_socket(i, internal_protocol, (TEST_PORT+i), 0x00);
   printf("\r\nSocket:%d Port:%d\r\n", i, (TEST_PORT+i));
...
int connect(int s, const struct sockaddr *name, socklen_t namelen)
{
   ...
   ret = wiznet_connect(s, destip, destport);
...
int recvfrom(int s, void *mem, size_t len, int flags, struct sockaddr *from, socklen_t *fromlen)
{
   ...
   ret = wiznet_recvfrom(s, mem, len, destip, (uint16_t*)&destport);
...
int send(int s, const void *dataptr, size_t size, int flags)
{
   ...
   ret = wiznet_send(s, (uint8_t *)dataptr, size);
...
int sendto(int s, const void *dataptr, size_t size, int flags, const struct sockaddr *to, socklen_t tolen)
{
   ...
   ret = wiznet_sendto(s, (uint8_t *)dataptr, size, destip, destport);
